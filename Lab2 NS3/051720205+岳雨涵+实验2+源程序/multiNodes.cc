...
main(int argc,char *argv[])
{
    ...
    AdHocDevices = wifi.Install(phy,mac,AdHocNode);  //安装协议栈
  ...
  phy.EnableAsciiAll (ascii.CreateFileStream ("hoc.tr"));  //对数据包进行追踪用于后续数据处理
 
    Simulator::Stop(Seconds(15.0));
    phy.EnablePcap ("hocwifi", AdHocDevices.Get (0)); //查看具体数据包内容
